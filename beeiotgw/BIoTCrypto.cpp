...
{
	BHLOG(LOGLORAW) printf("  LoRaMacComputeMic: buf=0x%02X%02X%02X%02X..., size=%i", 
       buffer[0], buffer[1], buffer[2], buffer[3], size);
    BHLOG(LOGLORAW) printf(", key=0x%02X%02X%02X%02X..., address=0x%X\n", key[0],key[1],key[2],key[3], address);
    BHLOG(LOGLORAW) printf("  LoRaMacComputeMic: dir=%i, SCnt=0x%02X\n", dir, sequenceCounter);
                    ...
                    uint32_t sequenceCounter, uint8_t *encBuffer )
{
    ...
    while( size >= 16 )
    {
        BHLOG(LOGLORAW) printf("  LoRaMacPayloadEncrypt: BUF=0x");
        aBlock[15] = ( ( ctr ) & 0xFF );
        ctr++;
        ...
        for( i = 0; i < 16; i++ )
        {
            ...
            BHLOG(LOGLORAW) printf("%02X",encBuffer[bufferIndex+i]);
        }
        size -= 16;
        bufferIndex += 16;
...
{
	BHLOG(LOGLORAW) printf("  LoRaMacJoinComputeMic: buf=0x%02X%02X%02X%02X..., size=%i", 
       buffer[0], buffer[1], buffer[2], buffer[3], size);
    BHLOG(LOGLORAW) printf(", key=0x%02X%02X%02X%02X...\n", key[0],key[1],key[2],key[3]);
...
void LoRaMacJoinDecrypt( const uint8_t *buffer, uint16_t size, const uint8_t *key, uint8_t *decBuffer )
{
    ...
    BHLOG(LOGLORAW) printf("  LoRaMacJoinDecrypt: decBuffer 0x%X %X %X %X %X %X ...\n",
...
void LoRaMacJoinComputeSKeys( const uint8_t *key, const uint8_t *appNonce, uint16_t devNonce, uint8_t *nwkSKey, uint8_t *appSKey )
{
    ...
    BHLOG(LOGLORAW) printf("  LoRaMacJoinComputeSKeys: appSKey: 0x%X %X %X %X %X %X...\n",
